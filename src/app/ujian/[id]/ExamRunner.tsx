...
export default function ExamRunner({ attemptId }: { attemptId: string }) {
  ...
  // Referensi sinkronisasi waktu server (agar timer tidak bergantung jam client).
  ...
  const submitSection = useCallback(
    async (sectionId: string) => {
      ...
      try {
        ...
        /* diabaikan; state akan disinkronkan ulang */
      }
      await loadState();
    },
    [attemptId, loadState],
  ...
  // Auto-submit ketika waktu subtes habis.
  useEffect(() => {
  ...
  async function pilihJawaban(q: Question, optionId: string | null) {
    ...
    if (res.status === 409) {
      // Waktu habis di sisi server → sinkronkan.
      submitSection(state.active.sectionId);
    }
  }
  ...
  if (error) {
    return (
      <main className="flex flex-1 items-center justify-center p-8">
        <div className="rounded-xl bg-white p-6 text-center shadow ring-1 ring-slate-200">
  ...
  const terjawab = active.questions.filter((x) => x.pilihanOptionId).length;
    ...
    <main className="flex flex-1 flex-col">
      ...
      <div className="sticky top-0 z-20 border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
          <div className="min-w-0">
            <p className="truncate text-xs text-slate-500">{state.packageNama}</p>
      ...
      <div className="mx-auto grid w-full max-w-6xl flex-1 gap-6 px-4 py-6 lg:grid-cols-[1fr_20rem]">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6">
          ...
          <div
            className={`mt-4 whitespace-pre-line text-lg leading-relaxed text-slate-900 ${
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-5 lg:sticky lg:top-20 lg:self-start">
          <h3 className="text-sm font-semibold text-slate-700">Navigasi Soal</h3>
          ...
          <div className="mt-4 grid grid-cols-6 gap-2 lg:grid-cols-5">
            {active.questions.map((x, i) => {
              const isCur = i === idx;
              let cls = "border border-slate-300 bg-white text-slate-600";
              if (x.ragu) cls = "bg-amber-400 text-white border-amber-400";
              else if (x.pilihanOptionId)
              ...
              return (
                <button
                  ...
                  className={`h-9 rounded text-sm font-semibold transition ${cls} ${
                    isCur ? "ring-2 ring-slate-800 ring-offset-1" : ""
                  }`}
                >
                  {x.urutan}
                </button>
          ...
          <div className="mt-4 space-y-1 text-sm text-slate-500">
            ...
            {raguCount > 0 && (
              <p className="text-amber-600">Ragu-ragu: {raguCount}</p>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              ...
              {raguCount > 0 && (
                <> Masih ada {raguCount} soal ditandai ragu-ragu.</>
              )}
              <br />
